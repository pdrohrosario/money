...
public interface ObjetivoRepository extends JpaRepository<Objetivo, Long>
{
	...
	@Query(value = "select new com.money.model.dto.ObjetivoDetalheDTO(g.id, p.titulo, t.nome ,u.userName, p.quantia, p.dataInicio, p.dataFim, p.descricao) from Objetivo g "
	...
	@Modifying
	@Transactional
	@Query(value = "INSERT INTO objetivo (id, tipo_gasto_id, plano_gasto_id, user_id)"
		+ " VALUES (:id, :tipo_gasto_id, :plano_gasto_id, :user_id);", nativeQuery = true)
	void saveGoal(@Param("id") Long id, @Param("plano_gasto_id") Long planoGastoId, @Param("user_id") Long userId,
...
//			+ " inner join Objetivo g on t.tipoGasto.id = g.tipoGasto.id and g.id = :objetivoId"
...
//	ObjetivoStatusDTO verifyGoalStatus(@Param("objetivoId") Long objetivoId, @Param("userId") Long userId);
	...
	ObjetivoStatusDTO verifyGoalStatus(@Param("objetivoId") Long objetivoId);
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE objetivo g SET quantia = :quantia, description = :description, end_date = :dataFim WHERE g.id = :objetivoId ", nativeQuery = true)
